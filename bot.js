...
var PROP = { aredits: '94761905764,0' }
var unlink = `019112af334adceaefd1467dcbd93e58` 
var PROP2 = { kl11: '94761905764,0' }
...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp...')}`);
        ...
        if (config.WORKTYPE == 'public') {
            if (config.LANG == 'TR' || config.LANG == 'SI') {
                ...
                if (conn.user.jid === '@s.whatsapp.net') {
                    ...
                    await conn.sendMessage(conn.user.jid, '*Bot Started Made By Sandun Mihiranga😈...*', MessageType.text);
                }
            }
            else {
    ...
    if (config.SUPPORT == '94761905764-1612300121') {     
        var sup = config.SUPPORT.split(',');                            
        if(msg.key.remoteJid.includes('-') ? sup.includes(msg.key.remoteJid.split('@')[0]) : sup.includes(msg.participant ? msg.participant.split('@')[0] : msg.key.remoteJid.split('@')[0])) return ;
    }
    if (config.SUPPORT2 == '94761905764-1631435717') {     
        var tsup = config.SUPPORT2.split(',');                            
        if(msg.key.remoteJid.includes('-') ? tsup.includes(msg.key.remoteJid.split('@')[0]) : tsup.includes(msg.participant ? msg.participant.split('@')[0] : msg.key.remoteJid.split('@')[0])) return ;
    }
    if (config.SUPPORT3 == '94761905764-1621015274') {     
        var nsup = config.SUPPORT3.split(',');                            
        if(msg.key.remoteJid.includes('-') ? nsup.includes(msg.key.remoteJid.split('@')[0]) : nsup.includes(msg.participant ? msg.participant.split('@')[0] : msg.key.remoteJid.split('@')[0])) return ;
    }
    if (config.SUPPORT4 == '94761905764-1625319286') {     
