...
if (Config.WORKTYPE == 'private') {
    ...
    Julie.addCommand({pattern: 'owner', fromMe: true, desc: 'shows the detail of bot owner'}, (async (message, match) => {
        ...
        if (Config.OWNER == 'default') {
            await message.client.sendMessage(message.jid,'*Toxic-Alexa created by Sandun Mihiranga*' , MessageType.text);
        }
        else {
            await message.client.sendMessage(message.jid,Config.OWNER + '\n Toxic-Alexa created by Sandun Mihiranga', MessageType.text);
        }
    }));
}
